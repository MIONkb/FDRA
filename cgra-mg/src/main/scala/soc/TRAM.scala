...
class Tram(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(opcodes, nPTWPorts = 1) {
  ...
  val hasMask = true // false // spadDataWidth != cgraDataWidth
  println(s"hasMask: $hasMask")
...
class TramModuleImp(outer: Tram)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
  ...
  val loader = Module(new LoadController(spadAddrWidth, spadAddrNum, lgMaxDataLen, spadDataWidth, cgraDataWidth, hasMask, idWidth, streamQueDepth, loadQueDepth))
  val storer = Module(new StoreController(spadAddrWidth, lgMaxDataLen, spadDataWidth, cgraDataWidth, hasMask, idWidth, streamQueDepth))
