package tram

import chisel3._
import chisel3.util._
import freechips.rocketchip.tile._
//import freechips.rocketchip.config._
import freechips.rocketchip.rocket._
import org.chipsalliance.cde.config._
import TramISA._


/** Load controller, transfer data from remote memory (virtual address) to local scratchpad (physical address)
  * @param spadAddrWidth  scratchpad address width, <= 32
  * @param spadAddrNum    scratchpad address number, be multiple to support broadcast write
  * @param lgMaxDataLen   log2(the max data length in one DMA request)
  * @param busdataWidth   system bus data width in bits
  * @param customDataWidth  system bus data width in bits
  * @param hasMask        if has mask signal in the stream interface
  * @param idWidth        width of the ID for a DMA request
  * @param streamQueDepth stream queue depth, can be zero
  * @param reqQueDepth    request queue depth
  */
class LoadController(spadAddrWidth: Int, spadAddrNum: Int, lgMaxDataLen: Int, busdataWidth: Int, customerDataWidth: Int, hasMask: Boolean, idWidth: Int, streamQueDepth: Int, reqQueDepth: Int)
                    (implicit p: Parameters) extends CoreModule {
  val io = IO(new Bundle {
    val core = new CoreIF(idWidth)
    val dma = Flipped(new DMAStreamReadIF(lgMaxDataLen, busdataWidth, hasMask, idWidth))
    val spad = Flipped(new SpadStreamWriteIF(spadAddrWidth, spadAddrNum, lgMaxDataLen, busdataWidth, hasMask, idWidth))
  })

  // // decouple the DMA requests with the data streams
  // class SpadReq(spadAddrWidth: Int, spadAddrNum: Int, lgMaxDataLen: Int, idWidth: Int) extends Bundle{
  //   val addrs = Vec(spadAddrNum, UInt(spadAddrWidth.W))   // start addresses
  //   val num = UInt(log2Ceil(spadAddrNum).W) // used address number - 1
  //   val len = UInt(lgMaxDataLen.W)
  //   val id = UInt(idWidth.W)
  // }
  // decouple the DMA requests with the data streams 
  /// add bias_elem_num to support unaligned request MionLou 20241112
  class SpadReq(spadAddrWidth: Int, spadAddrNum: Int, lgMaxDataLen: Int, idWidth: Int, biasElemNumWidth: Int) extends Bundle{
    val addrs = Vec(spadAddrNum, UInt(spadAddrWidth.W))   // start addresses
    val num = UInt(log2Ceil(spadAddrNum).W) // used address number - 1
    val len = UInt(lgMaxDataLen.W)
    val id = UInt(idWidth.W)
    val bias_elem_num = UInt(biasElemNumWidth.W)
  }

  val spadReqQue = Module(new Queue(new SpadReq(spadAddrWidth, spadAddrNum, lgMaxDataLen, idWidth, log2Ceil(busdataWidth/customerDataWidth)), reqQueDepth))

  val dataByte = busdataWidth/8
  val s_idle :: s_dma_req :: Nil = Enum(2)
  val state = RegInit(s_idle)


  val remote_addr = RegInit(0.U(coreMaxAddrBits.W)) // real remote DRAM virtual address

//  val spad_addr = RegInit(0.U(spadAddrWidth.W)) // scratchpad address from rs1
  val spad_addrs = RegInit(VecInit(Seq.fill(spadAddrNum){0.U(spadAddrWidth.W)})) // scratchpad address from rs1
  val addr_num = RegInit(0.U(log2Ceil(spadAddrNum).W))
  // val total_len = RegInit(0.U(lgMaxDataLen.W)) // total data length
  val fused = RegInit(false.B) // can be fused with next load command, with the same remote_addr, total_len
  val status = Reg(new MStatus)
  val id = RegInit(0.U(idWidth.W))
  val rs1 = io.core.req.bits.cmd.rs1.asTypeOf(new LoadRs1)
  val rs2 = io.core.req.bits.cmd.rs2.asTypeOf(new LoadRs2)

  // Mion Lou, 20240410:
  // Try to fixed the bug: 
  // the mismatch between address of DMA load request and
  // the system bus bitwidth.
  // For example, the system bus bitwidth is 128 bits, and 
  // I want to deliver a 32-bit and 9 datas stream from 82000004 to 8200028,
  // this will lead to a bug. 
  val request_addr = Cat(rs1.remote_addr(coreMaxAddrBits - 1, log2Ceil(busdataWidth/8)), 0.U(log2Ceil(busdataWidth/8).toInt.W))
  //  request_addr: request DRAM virtual address recieved from Reservation Station
  // val dataByte_num = log2Ceil(dataByte)


  val dma_request_len = RegInit(0.U(lgMaxDataLen.W)) // total data length for dma request
  val spad_needed_len = RegInit(0.U(lgMaxDataLen.W)) // total data length for spad 
  // val bias_bits = RegInit(0.U(log2Ceil(busdataWidth).W)) 

  val next_bias_element_num_wire = rs1.remote_addr(log2Ceil(busdataWidth/8) - 1, log2Ceil(customerDataWidth/8))
  // val next_bias_element_num_wire = rs1.remote_addr(busdataWidth/8 - 1, customerDataWidth/8)
  val bias_element_num = RegInit(0.U(log2Ceil(busdataWidth/customerDataWidth).W))
  val next_bias_element_num_buffer = RegInit(0.U(log2Ceil(busdataWidth/customerDataWidth).W))
  val bias_bits = Cat(bias_element_num, 0.U(log2Ceil(customerDataWidth).W)) // the bias to bus transfer

  switch(state){
    is(s_idle){
      when(io.core.req.fire){
        // remote_addr := rs1.remote_addr
        remote_addr := request_addr
        spad_addrs(addr_num) := rs2.spad_addr

        dma_request_len := Mux(next_bias_element_num_wire === 0.U, rs2.total_len, rs2.total_len + dataByte.U) 
        next_bias_element_num_buffer := next_bias_element_num_wire // Mion Lou add

        spad_needed_len := rs2.total_len
        fused := rs2.fused.asBool
        status := io.core.req.bits.cmd.status
        id := io.core.req.bits.id
        when(rs2.fused.asBool && addr_num < (spadAddrNum-1).U){
          state := s_idle
          addr_num := addr_num + 1.U
        }.otherwise{
          state := s_dma_req
        }
      }
    }
    is(s_dma_req){
      when(io.dma.req.fire){
        state := s_idle
        addr_num := 0.U
      }
    }
  }

  io.core.req.ready := (state === s_idle)
  // DMA request
  io.dma.req.valid := (state === s_dma_req) && spadReqQue.io.enq.ready
  io.dma.req.bits.addr := remote_addr
  io.dma.req.bits.len := dma_request_len
  io.dma.req.bits.id := id
  io.dma.req.bits.status := status
  // Cache request in Queue
  spadReqQue.io.enq.valid := (state === s_dma_req) && io.dma.req.ready
  spadReqQue.io.enq.bits.addrs := spad_addrs
  spadReqQue.io.enq.bits.num := addr_num
  spadReqQue.io.enq.bits.len := spad_needed_len
  spadReqQue.io.enq.bits.id := id
  spadReqQue.io.enq.bits.bias_elem_num := next_bias_element_num_buffer

  // Data streams
  val s_spad_req :: s_spad_prepare :: s_spad_stream :: s_exp :: s_resp :: Nil = Enum(5)
  val spadState = RegInit(s_spad_req)
  val leftLen = RegInit(0.U(lgMaxDataLen.W)) // left data length
  val spadReqId = RegInit(0.U(idWidth.W))
  val success = RegInit(true.B)
  val leftNum = RegInit(0.U(log2Ceil(spadAddrNum).W))

  // Mion Lou add:
  val elements_buffer = RegInit(0.U((busdataWidth).W))
  val elements_queue = Wire(UInt((2 * busdataWidth).W))
  val elements_queue_shift = Wire(UInt((2 * busdataWidth).W))

  val output_stream = Wire(Decoupled(new DMAStream(busdataWidth, hasMask, idWidth)))
  // val spadStream_valid = RegInit(false.B)

  elements_queue := Cat(io.dma.stream.bits.data, elements_buffer) 
  elements_queue_shift := (elements_queue >> bias_bits) // address space grows upwards  
  // val output = Wire(UInt(busdataWidth.W))

  switch(spadState){
    is(s_spad_req){
      when(io.spad.req.fire){
        spadState := s_spad_prepare //Mion Lou
        elements_buffer := io.dma.stream.bits.data
        leftLen := spadReqQue.io.deq.bits.len
        leftNum := spadReqQue.io.deq.bits.num
        spadReqId := spadReqQue.io.deq.bits.id
        bias_element_num := spadReqQue.io.deq.bits.bias_elem_num
        success := true.B
      }
    }
    //Mion Lou add
    is(s_spad_prepare){
      when(io.dma.stream.fire){
        spadState := Mux(bias_element_num === 0.U & leftLen <= dataByte.U, s_resp, s_spad_stream)
        elements_buffer := io.dma.stream.bits.data
        leftLen := Mux(bias_element_num === 0.U, leftLen - dataByte.U , leftLen)
      }
    }
    is(s_spad_stream){
      when(io.dma.exp.req){
        spadState := s_exp
        success := false.B
      }.elsewhen(io.dma.stream.fire){
        elements_buffer := io.dma.stream.bits.data
        leftLen := leftLen - dataByte.U
        spadState := Mux(leftLen <= dataByte.U, s_resp, s_spad_stream)
      }
      // .elsewhen(!io.dma.stream.fire){
      //    /// @jhlou: This might be wrong
      //   elements_buffer := elements_buffer
      //   leftLen := leftLen
      //   spadState := Mux(leftLen < dataByte.U, 
      //                           Mux(bias_element_num === 0.U, s_spad_stream, s_resp), 
      //                           s_spad_stream)
      // }
    }
    is(s_exp){
      when(io.spad.exp.ack){
        spadState := s_resp
      }
    }
    is(s_resp){
      when(io.core.resp.fire){
        leftNum := leftNum - 1.U
        when(leftNum === 0.U){
          spadState := s_spad_req
        }
      }
    }
  }
  // Mion Lou add:
  output_stream.bits.data := Mux(bias_element_num === 0.U, io.dma.stream.bits.data, elements_queue_shift(busdataWidth - 1, 0))
  output_stream.bits.mask := io.dma.stream.bits.mask
  output_stream.bits.id   := io.dma.stream.bits.id
  output_stream.bits.last := io.dma.stream.bits.last
  io.dma.stream.ready := output_stream.ready 
  output_stream.valid := ((bias_element_num === 0.U && spadState === s_spad_prepare) || spadState === s_spad_stream) && io.dma.stream.valid

  // read request from Queue
  spadReqQue.io.deq.ready := (spadState === s_spad_req) && io.spad.req.ready
  // Scratchpad request
  io.spad.req.valid := (spadState === s_spad_req) && spadReqQue.io.deq.valid
  io.spad.req.bits.addrs := spadReqQue.io.deq.bits.addrs
  io.spad.req.bits.num := spadReqQue.io.deq.bits.num
  io.spad.req.bits.len := spadReqQue.io.deq.bits.len
  io.spad.req.bits.id := spadReqQue.io.deq.bits.id
  // Exception
  io.spad.exp.req := (spadState === s_exp)
  io.spad.exp.id := spadReqId
  io.dma.exp.ack := io.spad.exp.ack 
  // data stream
  if(streamQueDepth == 0){
    // io.dma.stream.ready := io.spad.stream.ready && (spadState === s_spad_stream)
    output_stream.ready := io.spad.stream.ready && (spadState === s_spad_stream || spadState === s_spad_prepare)
    // io.spad.stream.valid := io.dma.stream.valid && (spadState === s_spad_stream)
    io.spad.stream.valid := output_stream.valid /*&& (spadState === s_spad_stream)*/
    // io.spad.stream.bits := io.dma.stream.bits
    io.spad.stream.bits := output_stream.bits
  }else{
    val que = Module(new Queue(new DMAStream(busdataWidth, hasMask, idWidth), streamQueDepth,
      false, false, false, true))
    // io.dma.stream.ready := que.io.enq.ready && (spadState === s_spad_stream)
    output_stream.ready := que.io.enq.ready && (spadState === s_spad_stream || spadState === s_spad_prepare)
    // que.io.enq.valid := io.dma.stream.valid && (spadState === s_spad_stream)
    que.io.enq.valid := output_stream.valid /*&& (spadState === s_spad_stream)*/
    // que.io.enq.bits := io.dma.stream.bits
    que.io.enq.bits := output_stream.bits
    io.spad.stream <> que.io.deq
    que.io.flush.get := io.spad.exp.req
  }
  // response
  io.core.resp.valid := (spadState === s_resp)
  io.core.resp.bits.success := success
  io.core.resp.bits.id := spadReqId

}