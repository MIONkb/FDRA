...
//  val writeBankIdx = writeAddr(spadAddrWidth-1, lgSizeBank)
  ...
  // val nextReadAddr = Wire(spadAddrWidth.W) 
  val preReadAddr = RegNext(readAddr) /// mion lou fix 20250507
  ...
  switch(readState){
    is(s_read_idle){
      ...
      when(io.read.req.fire){
        ...
        readState := Mux(io.read.req.bits.len > (dataByte/8).U, s_read_data, s_read_data_last) // @jhlou 20250722
    ...
    is(s_read_data){
      ...
      // .otherwise{ /// mion lou fix
      //   rdataValid := Mux(readAddr >= readMaxAddr, true.B, false.B)
      //   readBankIdxSyn := readBankIdx
      //   readAddr  := preReadAddr 
      //   readState := Mux(readAddr >= readMaxAddr, s_read_data_last, s_read_data)
      // }
    }
...
//      }
  ...
  if(hasMask) {
    val readMask = Wire(UInt(io.read.stream.bits.mask.getWidth.toInt.W))
    readMask := Mux(readAddr >= readMaxAddr, (1.U << (readAddr - readMaxAddr)).asUInt - 1.U, ((1 << dataByte)-1).U)
    // io.read.stream.bits.mask := Mux(readState === s_read_idle || readState === s_read_data_last,
    //                                 ((1 << dataByte)-1).U,
    //                                 RegEnable(readMask, io.read.stream.ready))
    io.read.stream.bits.mask := RegEnable(readMask, io.read.stream.ready)
  }
}