...
//}
class FDIV_(val expWidth: Int, val precision: Int) extends Module {
  val io = IO(new FDIVIO(expWidth, precision))
  io.result := DontCare
  io.specialIO.out_valid := DontCare
  io.specialIO.in_ready  := DontCare
}

class FDIV(val expWidth: Int, val precision: Int, val Debug_Sim : Boolean = false) extends Module {
  val io = IO(new FDIVIO(expWidth, precision))
  ...
  // @jhlou: If this assertion exists, then Chipyard cannot remove FDIV, even if we do not use its output.
  //         I think this is because chipyard regard this module as a debug module  
  // if(Debug_Sim) assert(outValidReg === state(s_post_0))
