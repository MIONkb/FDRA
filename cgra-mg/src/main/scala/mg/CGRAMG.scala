...
object CGRAMG extends App{
  ...
  if(loadSpec){
    val jsonFile = rootDirPath + "/generators/fdra/cgra-mg/src/main/resources/cgra_spec.json"
    TramSpec.dumpSpec(jsonFile)
    TramSpec.loadSpec(jsonFile)
  }
  TramSpec.attrs("dumpOperationSet") = dumpOperationSet
  if(dumpOperationSet){
    TramSpec.attrs("operation_set_filename") = rootDirPath + "/generators/fdra/cgra-mg/src/main/resources/operations.json"
  }
  TramSpec.attrs("dumpADG") = dumpADG
  if(dumpADG){
    TramSpec.attrs("cgra_adg_filename") = rootDirPath + "/generators/fdra/cgra-mg/src/main/resources/cgra_adg.json"
  }
  if(genVerilog){
