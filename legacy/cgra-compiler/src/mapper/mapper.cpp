...
// dumpCallFunc : dump CGRA execution call function
// dumpMappedViz : dump mapped visual graph
// resultDir: mapped result directory
// CallFuncWithExplicitArgs: Arguments of dumped call func has explicit arguments like 
//       "cgra_execute(void cin_0, void cout_0)" 
bool Mapper::execute(bool dumpCallFunc, bool dumpMappedViz, std::string resultDir, bool CallFuncWithExplicitArgs){
    ...
    if(res){
        ...
        if(dumpCallFunc){
            ...
            if(CallFuncWithExplicitArgs){
                /// add by Mion Lou 20230623
                std::ofstream func_ofs(dir + "/" + getAppName() + ".c");
                _sched->execute_withExplicitArgs(_mapping, func_ofs, getAppName());
                func_ofs.close();
            }
            else{
