...
// analyze dependence, allocate spad space, dump execution call function with explicit args
void IOScheduler::execute_withExplicitArgs(Mapping *mapping, std::ostream &os_func, const std::string appname)
{
    // dump necessary header file
    os_func << "#include \"include/ISA.h\"\n";
    ...
    */
    ...
    // dump execution call function head
    os_func << "void " << appname <<"(";
    
    // dump explicit args pass instead of "(void** din_addr, void** dout_addr)"
    std::vector<std::pair<int, int>> ld_data_deps; // <id, dep>
    std::vector<int> st_ids; // store node ids  
    for(auto &elem : _dfg_io_infos){
        if(elem.second.isStore){
    ...
    /// load cfg
    os_func << "\tload_cfg((void*)cin, 0x" << std::hex << cfgBaseAddrSpad << std::dec << ", " 
       << _cfg_len << ", " << _task_id << ", " << _ld_cfg_dep << ");\n";
    ...
    for(auto &elem : ld_data_deps){
        ...
        // void* remoteAddr = ioName2AddrLen[memRefName].first;        
        auto& ioInfo = _dfg_io_infos[elem.first];
        os_func << "\tload_data(din_" << i << ", 0x" << std::hex << ioInfo.addr << std::dec << ", " 
           << dataLen << ", " << fused << ", " << _task_id << ", " << ioInfo.dep << ");\n";
        i++;
    }

    /// config and execute
    os_func << "\tconfig(0x" << std::hex << cfgBaseAddrCtrl << std::dec << ", " << _cfg_num << ", " << _task_id << ", " << 0 << ");\n";
    os_func << "\texecute(0x" << std::hex << _iob_ens << std::dec << ", " << _task_id << ", " << _ex_dep << ");\n";
    ...
    for(auto id : st_ids){
        ...
        // int dataLen = ioName2AddrLen[memRefName].second;
        auto& ioInfo = _dfg_io_infos[id];
        os_func << "\tstore(dout_" << i << ", 0x" << std::hex << ioInfo.addr << std::dec << ", " 
           << dataLen << ", " << _task_id << ", " << 0 << ");\n";
        i++;
    }
    // return std::make_pair(ldArrayNames, stArrayNames);
